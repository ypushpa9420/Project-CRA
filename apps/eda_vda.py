...
def app():
    ...
    df= pd.read_pickle("./data/finaldataframe.pkl")
    df1= pd.read_pickle("./data/finaldataframe.pkl")
    dfo = pd.read_csv('./data/credit_risk_dataset.csv')
    st.subheader('**Below is the structure our original dataset**')
    oBuffer = io.StringIO()
    ...
    # columns
    st.subheader('**Below is the columns our original dataset**')
    st.text(dfo.columns)    
